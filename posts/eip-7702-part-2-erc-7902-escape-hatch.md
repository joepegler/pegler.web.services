...
Several mature smart-account implementations already provide versions of the capability we care about. [Kernel's EIP-7702 flow](https://docs.zerodev.app/get-started/eip-7702/quickstart) can attach Kernel account functionality to an EOA and support session keys. [Biconomy's Smart Sessions](https://docs.biconomy.io/sdk-reference/sessions) provide fine-grained policies for Nexus accounts, including an EIP-7702 delegate mode. [MetaMask's Delegation Framework](https://docs.metamask.io/smart-accounts-kit/guides/advanced-permissions/execute-on-metamask-users-behalf/) is a wallet-owned account stack offering another sophisticated permission model.

For an external wallet to permit delegation to Kernel, Nexus or another implementation, it would need to approve a particular deployment, audit scope, upgrade model and migration path rather than merely trust a brand name. If several wallets independently approved the same mature implementation, a dapp could build against one known account environment while users retained their existing EOAs and wallets.

Longer term, standards such as [ERC-7715](https://eips.ethereum.org/EIPS/eip-7715) and [ERC-7710](https://eips.ethereum.org/EIPS/eip-7710) aim closer to the interface the dapp actually wants: request a permission through the wallet and redeem it through a common onchain surface. If wallets converge on useful permission semantics, the dapp should stop caring whether Kernel, Nexus, MetaMask Delegator or another account implementation sits underneath.
